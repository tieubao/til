...
- [[oauth-refresh-token-rotation-one-grant-one-store]] - the same principle broken the other way: copying a single-holder OAuth grant into several stores instead of keeping it separate