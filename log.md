...
## [2026-09-17] ingest | Compile 12 notes merged directly via PR #634

Twelve notes landed via PR #634 (direct merge, bypassing Claude Code): `ai-tooling/a-prompt-constraint-is-a-request-not-an-enforcement`, `browser-automation/a-scripted-click-carries-no-user-activation` (new folder), `ci/matrix-context-is-empty-in-a-job-level-if` (new folder), `claude-code/skill-selection-runs-on-the-description-text-alone`, `cloudflare/wrangler-env-flag-deploys-a-second-worker`, `crypto/push-plus-reconcile-beats-polling-an-address-pool`, `devtools/a-self-updating-script-runs-the-old-bodies-for-one-tick`, `devtools/cargo-and-rustup-directories-are-not-caches`, `engineering/code-quality/an-escaped-delimiter-is-still-the-delimiter-byte`, `macos/optimize-mac-storage-turns-disk-pressure-into-an-icloud-loop`, `patterns/an-incremental-job-must-record-confirmed-absent`, `patterns/an-upstream-identifier-can-be-duplicated`.

Ran the compilation step on all twelve: checked for overlapping claims (none found beyond the deliberate cross-links below), added 20 backlinks total (12 outgoing from the new notes, 8 reciprocal on existing notes: `checks-that-report-success-while-verifying-nothing`, `commands-vs-hooks-vs-skills-decision-framework`, `durable-object-stubs-age-across-a-long-cron-tick`, `workers-logs-push-vs-poll-for-error-alerting`, `synthetic-ledger-identities-and-receipt-lookups`, `scope-boundary-bugs`). No contradictions with existing notes. No synthesis pages created (no cluster crossed the 4-note threshold from this batch alone). `index.md` and `README.md` updated for all twelve, plus two new Topics rows (`browser-automation/`, `ci/`) and a third (`cloudflare/`) that had never been added despite the folder holding notes since 2026-09-04.

Pre-existing index gap found and left alone (out of scope for this batch, flagged for a future `lint the wiki` pass): `crypto/synthetic-ledger-identities-and-receipt-lookups.md` and `engineering/code-quality/checks-that-report-success-while-verifying-nothing.md` were never added to `index.md`, and three of the four existing `cloudflare/` notes are still missing from `index.md`.

## [2026-09-23] ingest | ImageMagick 7 compositing gotchas from a logo-remix batch

Added to `devtools/`. Distilled from a batch of colored logo remixes generated from one white-on-black mask in ImageMagick 7, where each compositing bug cost a full rebuild. Four gotchas verified against a tiny synthetic mask and pixel/mean checks: `-flatten` composites using whatever `-compose` was last set (not `Over`), an `Overlay` composite's operand order plus blend mode drops the destination's alpha, `-rotate` needs `+repage` before a `-gravity`/`-crop` or the crop reads the stale virtual canvas, and parsing `magick txt:` output with `awk -F'[(,)]'` needs `+0` on the y field since it comes out as a string. A fifth claimed gotcha (blur-then-threshold always erasing noise at 62%) only partially reproduced: the clustering-near-50%-mean claim holds, but the failure is generic to any threshold far from a tight distribution's mean, not specific to 62%; recorded as a non-repro.

Privacy strip applied: no client, org, or personal paths in the note; all commands run against a synthetic circle/rectangle mask generated in-session. `index.md` and `README.md` updated under `devtools/`. Added a reciprocal backlink on `engineering/code-quality/an-escaped-delimiter-is-still-the-delimiter-byte.md` (same hand-split-without-coercion failure class).
