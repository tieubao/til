...
- [OAuth refresh tokens die when a grant is copied into several stores](notes/security/oauth-refresh-token-rotation-one-grant-one-store.md) - a rotating refresh token is a single-holder credential; copying one grant into ten agent-profile stores staggered a nine-day die-off as each copy replayed a rotated-away token
